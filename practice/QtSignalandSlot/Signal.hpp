...
#define MYSIG_FUN(_fun, ...) [=] (auto &&...__t) { return _fun(__VA_ARGS__ __VA_OPT__(,) std::forward<decltype(_t)>(_t)...); }
#else
#define MYSIG_FUN(_fun) [=] (auto &&..._t) { return __fun(std::forward<decltype(_t)>(_t)...); }
#endif
