...
struct Input {
    ...
    MySig::Signal<int> on_input;
    MySig::Signal<std::string, std::string> on_exit;
};
...
void dummy(Input& input) {
    ...
    input.on_input.connect(std::weak_ptr<Bar>(bar), &Bar::on_input, MySig::nshot_t(10)); // 调用 10 次后回调自动无效化
    gbar = bar;
